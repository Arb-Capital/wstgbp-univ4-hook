...
/// @title FeeMath (wstGBP/USDC venue)
/// @notice Pure fee computation for the wstGBP/USDC dynamic-fee hook: a directional base fee plus a
...
///      `deviationPpm = pool(wstGBP-per-USDC) / fair − 1`, so
///      - `d > 0` — the pool prices USDC *rich* (wstGBP cheap); the closing flow sells USDC — buys
///        wstGBP — and the redeem side pays. This is the post-NAV-ratchet conveyor arb.
///      - `d < 0` — the pool prices USDC *cheap* (wstGBP rich); the closing flow sells wstGBP
///        (mint side pays).
///      Flow pushing the pool away from fair value, or trading inside the threshold band, pays base only.
///
///      This venue's `FeeParams` has NINE fields (one oracle feed ⇒ one staleness window), vs the
///      WETH venue's ten; `swapFee`/`surchargePpm` semantics are byte-identical and both venues are
///      pinned by the same shared sim vector table (`sim/tests/feemath_vectors.json`).
///
...
library FeeMath {
    ...
    /// @notice All nine fields are uint24 => 216 bits => exactly one storage slot in the hook.
    ///         Fees/threshold/cap in ppm; slope ppm-scaled multiplier; staleness in seconds
    ///         (uint24 max ≈ 194 days — ample for the 24h-heartbeat GBP/USD window).
    struct FeeParams {
        uint24 baseFeeMintSide; // wstGBP-in base fee
        uint24 baseFeeRedeemSide; // USDC-in base fee
    ...
    /// @param isMintSide true when the swap's input currency is wstGBP (mint side), false for USDC in.
    ...
    function surchargePpm(bool isMintSide, int256 deviationPpm, FeeParams memory p) internal pure returns (uint256) {
        // Informed flow trades toward fair value: d > 0 is closed by selling USDC (redeem side),
    ...
    ///      `surchargeCapPpm ≤ maxFee`, `deviationThresholdPpm ≤ 100_000`, staleness window nonzero.
    ///      The two bases are independently settable (asymmetric responsiveness is a legitimate
    ///      policy choice) — no mint-vs-redeem ordering is enforced.
