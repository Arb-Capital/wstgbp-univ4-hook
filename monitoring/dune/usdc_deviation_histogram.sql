-- wstGBP/USDC hook: |deviation| histogram (5 bps buckets) over the trailing 30d, split by
-- whether the surcharge regime was active (|d| > 10 bps threshold). Raw-log form; decoded
-- equivalent: <namespace>.UsdcWstGbpHook_evt_SwapFee.
--
-- VENUE NOTE: unlike the WETH venue, this pool's LEGITIMATE rest states are the wrapper band
-- edges (~1250 ppm from fair) and the sim-predicted dynamic equilibrium rests ~5300 ppm below
-- fair — expect the histogram mass well away from zero; that is design, not drift.
--
