-- XAUT/wstGBP hook: daily swap count + fee stats by direction (mint side = wstGBP in).
-- RAW-LOG FORM so it works from block one, before Dune decodes the ABI. Decoded-table
-- equivalent after ABI submission: <namespace>.XautWstGbpHook_evt_SwapFee.
--
...
SELECT
    ...
    IF(varbinary_to_uint256(topic1) = 1, 'mint (wstGBP in)', 'redeem (XAUT in)') AS direction,
    count(*) AS swaps,
