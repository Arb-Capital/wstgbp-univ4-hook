...
-- spot gold, and XAUt persistently trades ~0.5% BELOW the metal, so this pool RESTS at
-- d ≈ −5000 ppm — mass in the ~50 bps bucket with below_fair = true is BY DESIGN. Investigate a
-- REGIME SHIFT (the rest mass migrating buckets, or flipping to below_fair = false), never the
-- rest mass itself. The sign split exists exactly to make that distinction visible.
--
...
-- At the −basis rest state that means XAUT-in swaps pay base only while wstGBP-in swaps pay
-- base + surcharge (the sweep winner's threshold, 1000 ppm, sits deliberately BELOW the basis —
...
SELECT
    ...
    deviation_ppm < 0 AS below_fair, -- the basis rest state lives on the NEGATIVE side
