-- WETH/wstGBP hook: daily swap count + fee stats by direction (mint side = wstGBP in).
-- RAW-LOG FORM so it works from block one, before Dune decodes the ABI. Decoded-table
-- equivalent after ABI submission: <namespace>.WethWstGbpHook_evt_SwapFee.
--
...
--
-- param: {{hook_address}} — the deployed WethWstGbpHook

SELECT
    date_trunc('day', block_time) AS day,
    IF(varbinary_to_uint256(topic1) = 1, 'mint (wstGBP in)', 'redeem (WETH in)') AS direction,
    count(*) AS swaps,
