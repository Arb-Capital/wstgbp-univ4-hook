#!/usr/bin/env bash
# Off-chain oracle-health probe for the WETH/wstGBP AND wstGBP/USDC venues — watches the ROOT
# CAUSE the on-chain events can't (SwapFee/OracleFallback only emit on swaps; a quiet pool in
# fallback is invisible). One script serves both venues: GBP/USD + navprice are shared inputs;
# ETH/USD is weth-only; USDC/USD is an ADVISORY depeg probe for the usdc venue (which reads no
# USDC feed on-chain — a depeg is invisible to that hook by design, SECURITY_USDC_WSTGBP.md §6;
# the runbook on alert is the owner pause).
