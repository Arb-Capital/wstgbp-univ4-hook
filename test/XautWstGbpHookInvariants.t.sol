...
/// @notice Stateful (invariant) suite for the XAUT/wstGBP dynamic-fee hook. A `XautHookHandler`
///         drives long randomly-interleaved sequences of swaps (all four modes + deviation-closing
///         arb), third-party LP add/remove, oracle drift/breakage/healing on all three legs (XAU/USD
///         + GBP/USD feeds + NAV), time warps, pause toggles, and owner fee-retunes, and the
///         invariants assert the cross-sequence properties the stateless suites can't reach:
///
...
contract XautWstGbpHookInvariants is XautWstGbpForkBase {
    using StateLibrary for IPoolManager;

    XautHookHandler internal handler;
    ...
    function setUp() public override {
        ...
        vm.etch(XAU_USD_FEED, address(impl).code);
        ...
        SettableFeed(XAU_USD_FEED).setReverting(false);
        SettableFeed(GBP_USD_FEED).setReverting(false);
        SettableFeed(XAU_USD_FEED).set(XAU_USD_ANSWER, block.timestamp);
        SettableFeed(GBP_USD_FEED).set(GBP_USD_ANSWER, block.timestamp);

        // The etched oracles must compose to the exact same live fair the pool was initialized at.
        ...
        assertEq(fair, 2000e18, "etched fair matches fixture fair");
        ...
        handler = new XautHookHandler(PM, key, hook, swapRouter, lpRouter, owner, wrapper.pip(), tickLower, tickUpper);

        // Endow the actor: wstGBP minted in base setUp plus dealt XAUT.
        IERC20Minimal(WSGEM).transfer(address(handler), 150_000 * WAD);
        deal(XAUT, address(handler), 300 * XAUT_UNIT);
        ...
        selectors[0] = XautHookHandler.mintExactIn.selector;
        selectors[1] = XautHookHandler.mintExactIn.selector;
        selectors[2] = XautHookHandler.redeemExactIn.selector;
        selectors[3] = XautHookHandler.redeemExactIn.selector;
        selectors[4] = XautHookHandler.mintExactOut.selector;
        selectors[5] = XautHookHandler.mintExactOut.selector;
        selectors[6] = XautHookHandler.redeemExactOut.selector;
        selectors[7] = XautHookHandler.redeemExactOut.selector;
        selectors[8] = XautHookHandler.arbToFair.selector;
        selectors[9] = XautHookHandler.arbToFair.selector;
        selectors[10] = XautHookHandler.driftXauUsd.selector;
        selectors[11] = XautHookHandler.driftGbpUsd.selector;
        selectors[12] = XautHookHandler.driftNav.selector;
        selectors[13] = XautHookHandler.breakOracle.selector;
        selectors[14] = XautHookHandler.healOracle.selector;
        selectors[15] = XautHookHandler.warpForward.selector;
        selectors[16] = XautHookHandler.addLiquidity.selector;
        selectors[17] = XautHookHandler.removeLiquidity.selector;
        selectors[18] = XautHookHandler.setPause.selector;
        selectors[19] = XautHookHandler.reTuneFeeParams.selector;
    ...
    function invariant_hookHoldsNoTokens() public view {
        ...
        assertEq(IERC20Minimal(XAUT).balanceOf(address(hook)), 0, "hook holds XAUT");
...
///         (wstGBP = currency0, fair ~2000 wstGBP/XAUT at the fixture's oracle values — XAUT
///         token counts run ~2000× smaller than the USDC venue's for the same value).
contract XautHookHandler is Test {
    using StateLibrary for IPoolManager;
    ...
    int256 internal lastXauAnswer = XAU_ANCHOR;
    ...
    function redeemExactIn(uint256 amt) public {
        amt = bound(amt, XAUT_UNIT / 1000, XAUT_UNIT); // XAUT in
        _observedSwap(!wstGbpIsC0, -int256(amt));
    }

    function mintExactOut(uint256 amt) public {
        amt = bound(amt, XAUT_UNIT / 2000, XAUT_UNIT / 2); // XAUT out
        _observedSwap(wstGbpIsC0, int256(amt));
    }
    ...
    function arbToFair() public {
        ...
        (uint256 fair, OracleLib.FallbackReason reason) = OracleLib.fairPriceWad(
            IAggregatorV3(address(xauFeed)),
            IAggregatorV3(address(gbpFeed)),
            wsgem,
            p.xauUsdStalenessSec,
        ...
        if (target > sqrtNow) {
            // price up = currency1 in (XAUT when wstGBP is currency0)
            uint256 amtIn = FullMath.mulDiv(liq, target - sqrtNow, FixedPoint96.Q96);
            amtIn = _min(amtIn, 6 * XAUT_UNIT);
    ...
    /// @dev Sign trap (fixture NatSpec): raising XAU/USD RAISES fair — the pool goes cheap (d < 0).
    function driftXauUsd(uint256 ppm) public {
        ppm = bound(ppm, 0, 60_000); // ±3% around the anchor (sign from parity)
        lastXauAnswer = _drifted(XAU_ANCHOR, ppm, 30_000);
        xauFeed.set(lastXauAnswer, block.timestamp);
    }

    /// @dev Sign trap (fixture NatSpec): raising GBP/USD LOWERS fair — the pool goes rich (d > 0).
    ...
    /// @dev One breakage mode per OracleLib FallbackReason family (both feeds + NAV).
    function breakOracle(uint8 mode) public {
        mode = uint8(bound(mode, 0, 7));
        if (mode == 0) xauFeed.setReverting(true); // XAU_FEED_CALL
        else if (mode == 1) xauFeed.set(0, block.timestamp); // XAU_FEED_ANSWER
        else if (mode == 2) xauFeed.set(int256(2e30), block.timestamp); // XAU_FEED_ANSWER (absurd)
        else if (mode == 3) xauFeed.set(lastXauAnswer, 1); // XAU_FEED_STALE (ancient round)
        else if (mode == 4) gbpFeed.setReverting(true); // GBP_FEED_CALL
        else if (mode == 5) gbpFeed.set(0, block.timestamp); // GBP_FEED_ANSWER
        else if (mode == 6) gbpFeed.set(lastGbpAnswer, 1); // GBP_FEED_STALE
        else vm.store(pip, PRICE_SLOT, bytes32(0)); // NAV_BAD (pip paused)
    }

    function healOracle() public {
        xauFeed.setReverting(false);
    ...
    /// @dev Valid-by-construction params (mirrors `checkParams` bounds); the staleness floors keep
    ...
    function reTuneFeeParams(uint256 seed) public {
        ...
        FeeMath.FeeParams memory p = FeeMath.FeeParams({
            ...
            xauUsdStalenessSec: uint24(bound(seed >> 128, 600, 100_000)),
    ...
    /// @dev Full independent mirror of `_beforeSwap` pricing from a FRESH oracle read (two-feed
    ///      composition). `word` is what the hook would write to its fair cache (1 = fallback verdict).
        ...
        returns (uint24 fee, uint256 word)
    {
        ...
        int256 d = OracleLib.deviationPpm(OracleLib.poolPriceWstGbpPerXautWad(sqrtBefore, wstGbpIsC0), fair);
        ...
        returns (uint24)
    {
        ...
        int256 d = OracleLib.deviationPpm(OracleLib.poolPriceWstGbpPerXautWad(sqrtBefore, wstGbpIsC0), cacheWord);
    ...
    ///      real-pair price caps (fair drifts at most ±~10% around ~2000 wstGBP/XAUT here).
    function _funded(bool zeroForOne, int256 amountSpecified, bool qOk, uint256 quoted) internal view returns (bool) {
        address tokenIn = zeroForOne ? (wstGbpIsC0 ? wsgem : xaut) : (wstGbpIsC0 ? xaut : wsgem);
        ...
        if (qOk) {
            ...
            // wstGBP in for XAUT out: ≤ 5e15 wstGBP-wei per XAUT-unit (≈ 5,000 wstGBP/XAUT, ~2.5x fair).
            need = uint256(amountSpecified) * 5e15;
        } else {
            // XAUT in for wstGBP out: ≤ 1 XAUT-unit per 8e14 wstGBP-wei (≈ 1/800 XAUT per wstGBP, ~2.5x fair).
            need = uint256(amountSpecified) / 8e14;
        }
        return bal >= need;
    }
    ...
    ///      the XAUT_UNIT decimal fold).
    function _fairSqrtPriceX96(uint256 fairWad) internal view returns (uint160) {
        uint256 ratioX192 = wstGbpIsC0 ? (XAUT_UNIT << 192) / fairWad : ((fairWad << 96) / XAUT_UNIT) << 96;
