...
/// @notice Pure unit suite for the USDC venue's FeeMath — no fork. Mirrors the sign-convention table
///         in the library NatSpec: d > 0 ⇒ USDC rich (wstGBP cheap) ⇒ redeem side (USDC-in) closes
///         and pays the surcharge; d < 0 ⇒ mint side (wstGBP-in) closes. All quantities ppm.
/// @dev The values below are deliberately IDENTICAL to the WETH suite's: `swapFee`/`surchargePpm`
///      are byte-identical across venues and both pin the same shared sim vector table — only the
///      struct shape (9 fields, single staleness window) differs.
contract UsdcWstGbpFeeMathTest is Test {
    uint256 constant PPM = 1e6;
    bool constant MINT = true;
    bool constant REDEEM = false;

    UsdcFeeMathHarness harness;

    function setUp() public {
        harness = new UsdcFeeMathHarness();
    }
    ...
    function test_directionGating() public pure {
        FeeMath.FeeParams memory p = _defaults();
        // d = +2000 (pool rich in USDC terms — wstGBP cheap): redeem side (USDC in) closes -> pays;
    ...
    ///      deliberately does not cover sim/), and the SAME table pins the WETH venue's FeeMath —
    ///      the two venues' swap-fee semantics may never drift apart. If either implementation
    ///      drifts, one of the suites fails. Fields: (isMintSide, deviationPpm, fee) over default
    ///      params unless noted.
