...
///         the adapter's quote == its execution == the independent {WsgemQuoter} across the WHOLE oracle
...
contract WsgemDirectAdapterFuzzTest is WsgemAdapterForkBase {
    uint256 internal constant NAV_LO = 0.01e18;
    uint256 internal constant NAV_HI = 100e18;
    uint256 internal constant BUY_CAP = 100_000 * 1e18; // gem in
    uint256 internal constant SELL_CAP = 300_000 * 1e18; // wsgem in (< the ~495k minted in setUp)

    function setUp() public override {
        super.setUp();
        // Ample balances so the parity sweep is never bounded by the actor's funds or the wrapper's gem
        // reserves at high NAV. Dealing gem touches neither price (oracle-driven) nor wsgem supply.
    ...
    function testFuzz_sellExactInParity(uint256 nav, uint256 amtIn) public {
        _setNav(bound(nav, NAV_LO, NAV_HI));
        amtIn = bound(amtIn, WAD, SELL_CAP); // >= redeem minimum (1 wsgem)
    ...
    function testFuzz_sellExactOutParity(uint256 nav, uint256 amtOut) public {
        _setNav(bound(nav, NAV_LO, NAV_HI));
        // Bound the gem output so the implied wsgem input (≈ amtOut/burncost) stays within both the
        // redeem minimum (1 wsgem) and the actor's ~495k holdings across the whole NAV range.
    ...
    function testFuzz_roundTripNeverProfits(uint256 nav, uint256 amtIn) public {
        ...
        assertLe(_bal(GEM, address(this)), t0, "round-trip cannot increase gem");
    }
    ...
    function testFuzz_exactOutBuyNoOvercharge(uint256 nav, uint256 amtOut) public {
        _setNav(bound(nav, NAV_LO, NAV_HI));
        amtOut = bound(amtOut, WAD, BUY_CAP);
        uint256 inExact = adapter.quoteExactOutput(GEM, amtOut); // ceil(amtOut * mintcost / WAD)
        // The wsgem actually delivered for `inExact` gem is >= amtOut and over-delivers only by bounded
        // dust; charging one wei less would under-deliver.
        uint256 deliveredFor = adapter.quoteExactInput(GEM, inExact);
        assertGe(deliveredFor, amtOut, "exact-out input covers the requested output");
        if (inExact > 0) {
            uint256 deliveredForLess = adapter.quoteExactInput(GEM, inExact - 1);
