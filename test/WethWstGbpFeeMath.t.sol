...
/// @notice Pure unit suite for FeeMath — no fork. Mirrors the sign-convention table in the library
///         NatSpec: d > 0 ⇒ WETH rich ⇒ redeem side (WETH-in) closes and pays the surcharge;
///         d < 0 ⇒ mint side (wstGBP-in) closes. All quantities ppm.
contract WethWstGbpFeeMathTest is Test {
    ...
    function test_directionGating() public pure {
        FeeMath.FeeParams memory p = _defaults();
        // d = +2000 (pool rich in WETH terms): redeem side closes -> pays; mint side opens -> base.
    ...
    /// @dev Spec Phase 1 acceptance: for arbitrary *valid* params and arbitrary deviation, the fee is
    ///      always inside [minFee, maxFee], fits under the override-flag bit, and never reverts.
    ...
    ///      deliberately does not cover sim/). If either implementation drifts, one of the two
    ///      suites fails. Fields: (isMintSide, deviationPpm, fee) over default params unless noted.
