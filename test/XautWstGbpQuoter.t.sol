...
///      driven from BOTH oracle legs — remember the two sign traps: XAU UP ⇒ fair UP ⇒ d < 0 (mint
///      side closes), GBP UP ⇒ fair DOWN ⇒ d > 0 (redeem side closes). Two feeds also means two
///      distinct bricked-feed fallback cases where the USDC template had one.
contract XautWstGbpQuoterTest is XautWstGbpForkBase {
    ...
    ///      tested (each quote is taken fresh against the then-current pool state). XAUT amounts are
    ///      value-matched to the USDC template's (~2000× smaller token counts at fair 2000 wstGBP/XAUT).
    function _assertAllModes() internal {
        _assertExactInParity(MINT_ZF1, uint128(10 * WAD)); // 10 wstGBP in
        _assertExactInParity(REDEEM_ZF1, uint128(XAUT_UNIT / 100)); // 0.01 XAUT in
        _assertExactOutParity(MINT_ZF1, uint128(XAUT_UNIT / 200)); // 0.005 XAUT out
    ...
    function test_parityClosingAboveThreshold() public {
        _mockFeed(XAU_USD_FEED, (XAU_USD_ANSWER * 101) / 100, block.timestamp); // fair +1% => d ~ -1%
        _assertAllModes();
    }
    ...
    function test_parityCapSaturated() public {
        _mockFeed(XAU_USD_FEED, (XAU_USD_ANSWER * 110) / 100, block.timestamp); // |d| >> cap
        _assertAllModes();
    }

    function test_parityFallbackXauFeedBricked() public {
        _brickFeed(XAU_USD_FEED);
        _assertAllModes();
    }

    function test_parityFallbackGbpFeedBricked() public {
    ...
    function testFuzz_parityAcrossRegimes(
        ...
        // Drive XAU/USD up to ±5% around the fixture default (spans base-only, ramp and cap regimes;
        // fair moves proportionally — the gold leg is the numerator).
        fairShiftPpm = bound(fairShiftPpm, 0, 50_000);
        int256 answer = int256(
            uint256(XAU_USD_ANSWER) * (shiftUp ? 1_000_000 + fairShiftPpm : 1_000_000 - fairShiftPpm) / 1_000_000
        );
        _mockFeed(XAU_USD_FEED, answer, block.timestamp);

        if (exactIn) {
            // Input sizes: wstGBP for the mint side, XAUT for the redeem side.
            uint128 amountIn = zeroForOne
                ? uint128(bound(size, WAD, 500 * WAD))  // 1..500 wstGBP
                : uint128(bound(size, XAUT_UNIT / 1000, XAUT_UNIT / 4)); // 0.001..0.25 XAUT
            _assertExactInParity(zeroForOne, amountIn);
        } else {
            uint128 amountOut = zeroForOne
                ? uint128(bound(size, XAUT_UNIT / 1000, XAUT_UNIT / 4))  // XAUT out
