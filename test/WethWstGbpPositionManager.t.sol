...
contract WethWstGbpPositionManagerTest is WethWstGbpForkBase {
    ...
    function setUp() public override {
        ...
        IERC20Minimal(WETH).approve(address(PERMIT2_ALLOWANCE), type(uint256).max);
        PERMIT2_ALLOWANCE.approve(WSGEM, address(POSM), type(uint160).max, type(uint48).max);
        PERMIT2_ALLOWANCE.approve(WETH, address(POSM), type(uint160).max, type(uint48).max);
    }
    ...
    function test_uiLifecycle_mintCollectExit() public {
        uint256 tokenId = _mint(tickLower, tickUpper, 1e21);
        assertEq(IERC721Minimal(address(POSM)).ownerOf(tokenId), address(this), "position NFT held by the funder");
        assertEq(_posmPositionLiquidity(tokenId, tickLower, tickUpper), 1e21, "liquidity live in the pool");
        ...
        SwapObservation memory redeem = _swapAndObserve(false, -int256(WAD / 20));
        ...
        uint256 collected1 = _bal(WETH, address(this)) - wethBefore;
        assertTrue(collected0 > 0 || collected1 > 0, "fees collected");
        assertEq(_posmPositionLiquidity(tokenId, tickLower, tickUpper), 1e21, "principal untouched by collect");

        // Full exit (the UI's "remove liquidity"): principal + any residual fees back to the wallet.
        wsgBefore = _bal(WSGEM, address(this));
        wethBefore = _bal(WETH, address(this));
        _decreaseAndTake(tokenId, 1e21);
        ...
        uint256 recoveredValueWsg =
            (_bal(WSGEM, address(this)) - wsgBefore) + (_bal(WETH, address(this)) - wethBefore) * fair / WAD;
